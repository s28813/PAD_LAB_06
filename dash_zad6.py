...
app.layout = html.Div([
    html.H4(children="wine quelity Dash"),
    generate_table(df),
    html.Br(),
    html.Label('Wybór typu wykresu'),
    dcc.RadioItems(['Regresja', 'Klasyfikacja'], 'Regresja', id = 'chart-radio'),
    html.Br(),
    html.Label('Wybór zmiennej'),
...
def update_graph(variable, chart):
    if chart == 'Regresja':
        ...
        fig = px.scatter(x=df['target'], y=df[variable], labels={"x": "target", "y": variable}, width=1000, height=600)
        fig.update_layout(margin={'l': 40, 'b': 40, 't': 10, 'r': 0}, hovermode='closest')
        return fig     
